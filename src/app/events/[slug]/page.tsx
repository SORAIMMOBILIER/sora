...
function formatEventDate(date: string | undefined, timeZone: string | undefined, locale: string, fallback: string) {
...
function formatEventTime(date?: string, timeZone?: string, locale: string = "fr") {
  if (!date) return null
  const formatted = new Intl.DateTimeFormat(locale === "en" ? "en-GB" : "fr-FR", {
    hour: "2-digit",
    minute: "2-digit",
  ...
  return locale === "en" ? formatted : formatted.replace(":", "h")
}

function timezoneHint(timeZone: string | undefined, locale: string, parisLabel: string, prefix: string) {
  if (!timeZone) return null
  if (timeZone === "Europe/Paris") return parisLabel
  const parts = timeZone.split("/")
  const city = parts[parts.length - 1]?.replace(/_/g, " ")
  return city ? `${prefix} ${city}` : null
}
...
export default async function EventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const eventTime = formatEventTime(event.startsAt, event.timezone, locale)
  const tzHint = timezoneHint(event.timezone, locale, t("parisTimeHint"), t("timeHintPrefix"))
        ...
        <header className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-16 items-end mb-14">
          <div className="md:col-span-7">
            <p className="eyebrow text-ink-muted mb-6">
              {event.eyebrow || event.eventType || t("defaultEyebrow")}
            </p>
          ...
          <aside className="md:col-span-5 bg-bg-soft border border-line rounded-sm p-6 md:p-8">
            <dl className="space-y-5">
              <div>
                <dt className="metadata text-ink/45 mb-1">{t("dateLabel")}</dt>
                <dd className="text-ink leading-relaxed">{formatEventDate(event.startsAt, event.timezone, locale, t("dateToBeAnnounced"))}</dd>
              </div>
              {eventTime && (
                <div>
                  <dt className="metadata text-ink/45 mb-1">{t("timeLabel")}</dt>
              ...
              {event.duration && (
                <div>
                  <dt className="metadata text-ink/45 mb-1">{t("durationLabel")}</dt>
                  <dd className="text-ink">{event.duration}</dd>
                </div>
              )}
              {event.location?.platform && (
                <div>
                  <dt className="metadata text-ink/45 mb-1">{t("locationLabel")}</dt>
                  <dd className="text-ink">{event.location.platform}</dd>
                </div>
              )}
              {event.status && (
                <div>
                  <dt className="metadata text-ink/45 mb-1">{t("statusLabel")}</dt>
        ...
        {(event.program?.length || event.speakers?.length) && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-start">
            ...
            {event.speakers && event.speakers.length > 0 && (
              <section className="bg-bg-soft border border-line rounded-sm p-6 md:p-8">
                <p className="eyebrow text-ink-muted mb-6">{t("speakersEyebrow")}</p>
        ...
        {showEmbeddedForm && (
          <section id="inscription" className="mt-16 md:mt-24 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start bg-bg-soft border border-line rounded-sm p-8 md:p-12">
            <div className="max-w-md">
              <p className="eyebrow text-ink-muted mb-6">{t("registrationEyebrow")}</p>
              ...
              {event.duration && (
                <p className="metadata text-ink/45 mt-6">{t("durationPrefix")} {event.duration}</p>
              )}
              {event.location?.platform && (
                <p className="metadata text-ink/45 mt-2">{t("locationPrefix")} {event.location.platform}</p>
              )}
            </div>
            <EventForm
              slug={event.slug}
